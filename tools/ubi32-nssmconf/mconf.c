...
"Overview\n"
"--------\n"
"Some Ultra distro features may be built directly into the image.\n"
...
"between different Ultra distro configurations.\n"
"\n"
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different Ultra Distro\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than\n"
	"Ultra Distro's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different Ultra Distro\n"
	"configurations available on a single machine.\n"
	"\n"
...
static char pwd[MAX_PATH_LENGTH];
...
/*
 * process_help()
 *	Substitutes the list tag in help with the appropriate list.
 */
static void process_help(char *list, char *help)
{
        char temp[MAX_NAME_LENGTH];
	int index;
        struct symbol *temp_sym;

	if (help[0] != '<' || strchr(help, '>') == NULL) {
                if (strlen(help) > MAX_HELP_LENGTH) {
                        fprintf(stderr, _("ERROR: Help text length more than %d characters.\n"), MAX_HELP_LENGTH);
                        exit(1);
                }
                snprintf(list, MAX_HELP_LENGTH, "%s", help);
                return;
        }

	for (index = 1; help[index] != '>' && index < MAX_NAME_LENGTH; index++) {
		temp[index-1] = help[index];
	}
	temp[index-1] = '\0';

        temp_sym = sym_find(temp);
        if (!temp_sym) {
                fprintf(stderr,_("Error: List %s not defined\n"), temp);
                exit(1);
        }
        if (strlen(list) + strlen(sym_get_string_value(temp_sym) + strlen("Available options : ")) > MAX_HELP_LENGTH) {
                fprintf(stderr,_("Error: Help text + list length more than %d characters.\n"), MAX_HELP_LENGTH);
                exit(1);
        }
        snprintf(list, MAX_HELP_LENGTH, "%sAvailable options : %s\n", strchr(help, '>')+1, sym_get_string_value(temp_sym));
}
...
static void show_help(struct menu *menu)
{
	...
	struct symbol *sym = menu->sym;
        char list[MAX_HELP_LENGTH];

	if (sym->help)
	{
		if (sym->name) {
			process_help(list, sym->help);
			str_printf(&help, "CONFIG_%s:\n\n", sym->name);
			str_append(&help, _(list));
			str_append(&help, "\n");
		}
	} else {
	...
	while (1) {
		...
		case 0:
			if (sym_set_string_value(menu->sym, input_buf))
				return;
			show_textbox(NULL, _("Symbol is overriden or you have made an invalid entry."), 5, 66);
			break;
		case 1:
			show_help(menu);
			break;
...
int main(int ac, char **av)
{
	...
	if (readlink("/proc/self/exe", pwd, MAX_PATH_LENGTH) == -1) {
		fprintf(stderr,_("Failed to get current working dir! Operating on /proc/self/exe\n"));
	}
	printf("%s: %s\n", av[0], BUILDATE);

	int len = strlen(pwd);
	strcpy(&pwd[len - 5], "lxdialog");
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("Ultra Distro %s Configuration"), 
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new Ultra Distro configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (av[2]) {
		if (conf_write(NULL, 1)) { /* Passing 1 as the second parameter will result in the generation of side files */
			fprintf(stderr, _("\n\n"
				"Error during writing of the Ultra Distro configuration.\n"
				"Your configuration changes were NOT saved."
				"\n\n"));
			return 1;
		} else {
		        printf(_("\n"
        		        "*** End of Ultra Distro configuration.\n"
               			"*** Execute 'make' to build the Ultra Distro or try 'make help'."
		                "\n")); 
			return 0;
		}
	} else {
                if (conf_write(NULL, 0)) { /* Passing 0 makes it behave Normal */
                        fprintf(stderr, _("\n\n"
                                "Error during writing of the Ultra Distro configuration.\n"
                                "Your configuration changes were NOT saved."
                                "\n\n"));
